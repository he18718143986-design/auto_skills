...
function main(): void {
  ...
  if (opts.fixturesDir) {
    ...
    for (const file of files) {
      ...
      const isDebugFixtures = /[/\\]fixtures[/\\]debug[/\\]?$/i.test(
        path.resolve(opts.fixturesDir!).replace(/\\/g, '/'),
      );
      let debugFeedbackLoopMode: 'off' | 'warn' | 'hard' | undefined;
      if (isDebugFixtures) {
        if (filename.startsWith('fail-')) {
          debugFeedbackLoopMode = 'hard';
        } else if (filename.startsWith('warn-')) {
          debugFeedbackLoopMode = 'warn';
        }
      }
      const result = verifyRule20(workflow, { debugFeedbackLoopMode });
