...
import { implHasDecisionRecordSourceForStages } from './workflow/StageInputSources';
import { isPrototypeTaskType } from './workflow/TaskType';
...
import { matchesIntegrationTestRunCommand } from './workflow/EntryScriptHeuristics';
...
export function isPrototypeWorkflow(wf: WorkflowDefinition): boolean {
  if (isPrototypeTaskType(wf.meta?.taskType)) {
    return true;
  }
  ...
  return (wf.stages ?? [])
    .filter(
      (s) =>
        ...
        (s.outputs ?? []).some((o) => o.key === PRIMARY_DECISION_OUTPUT_KEY),
    )
    .map((s) => s.id);
}
...
function isIntegrationTestRunStage(stage: Stage): boolean {
  ...
  return matchesIntegrationTestRunCommand(cmd, stage.id);
}
 ...
 *
 * 仅当三者皆不满足（create_sample 与 mock_data 之间毫无关联）才返回 mockData id。
 *
 * 说明：本判定**仅用于 warning**（M28.1 起 M27.2 不再生成期硬拦）——真实计划常经「决策记录 / 全局决策注入」
 * 共享 ASIN，生成期无结构化边可查，硬拦会误伤；ASIN 是否真正匹配的硬保证由 M21 运行期集成断言
 * （assert ≥1 query_status=success）在 test_run 落实。
 */
...
export function lintPrototypeDataContract(wf: WorkflowDefinition): string[] {
  ...
  // 1) 样例 / mock 数据是否共享同一份 ASIN 源（用 detectUnsharedSampleMockSource 的多信号判定，warning-only）
  const unsharedMockId = detectUnsharedSampleMockSource(wf);
  if (unsharedMockId) {
    warnings.push(formatContractWarningColon('sample-mock-source-unshared', unsharedMockId));
  }
  ...
  if (decisionStageIds.length > 0) {
    for (const stage of stages) {
      ...
      const refsDecision = implHasDecisionRecordSourceForStages(
        stage.input?.sources ?? [],
        decisionStageIds,
      );
      if (!refsDecision) {
        warnings.push(formatContractWarningColon('impl-missing-decision-source', stage.id));
      }
    }
  }
  ...
  for (const stage of integrationStages) {
    if (isWeakIntegrationAssertion(getCommand(stage))) {
      warnings.push(formatContractWarningColon('weak-integration-assertion', stage.id));
    }
  }
