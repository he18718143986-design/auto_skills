...
export function buildVerifyPythonImportsStage(opts: {
  ...
  return codeRunnerStage({
    id: opts.id,
    title: opts.title,
    description: '校验 Python 测试顶层 module import 在工作区存在。',
    command: buildNodeExtensionScriptCommand('verify-python-test-imports.mjs', opts.testFiles),
    dependsOn: opts.dependsOn,
  });
}
...
export function buildPythonFixIfFailedStage(opts: {
  ...
  const toolConfig: LlmTextConfig = {
    ...
    systemPrompt: [
      ...
      '步骤：1) 阅读 test_run 输出；2) 修复 import 路径、导出符号、requirements.txt；3) 对齐 test 与 impl API。',
      `可修改文件：${opts.writeTargets.join('、') || '工作区 .py 与 requirements.txt'}。`,
      '只输出需要写入的完整文件内容；禁止 Markdown 围栏。',
    ].join('\n'),
    writeOutputToFile: opts.writeTargets[0],
    writePathBase: 'workspace',
  };
