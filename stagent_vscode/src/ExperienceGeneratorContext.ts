...
import { EXPERIENCE_FAILURE_FEW_SHOT_MAX, EXPERIENCE_GEN_CONTEXT_MAX, EXPERIENCE_GEN_PICKED_MAX } from './UiListLimits';
...
function buildFailureExperienceFewShotForGenerator(
  ...
  const maxEntries = options?.maxEntries ?? EXPERIENCE_FAILURE_FEW_SHOT_MAX;
  let pool = experiences.filter(
    (e) =>
      e.completionStatus === 'failed' &&
      e.failureErrorType === ERROR_TYPE_TOOL_EXECUTION_FAILED &&
      isTestRunStageId(e.failureStageId ?? ''),
  );
  if (options?.taskType) {
...
export function buildExperienceFewShotForGenerator(
  ...
  const maxEntries = options?.maxEntries ?? EXPERIENCE_GEN_PICKED_MAX;
  ...
  const failureBlock =
    options?.includeFailures !== false
      ? buildFailureExperienceFewShotForGenerator(experiences, {
          maxEntries: EXPERIENCE_FAILURE_FEW_SHOT_MAX,
  ...
  if (picked.length > 0) {
    ...
    for (const exp of picked) {
      const outcomes = (exp.stageOutcomes ?? [])
        ...
        .slice(0, EXPERIENCE_GEN_CONTEXT_MAX)
        .map((o) => o.stageId)
        .join(', ');
      lines.push(
