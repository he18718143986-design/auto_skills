...
const CLASS_DEF_RE = /^\s*class\s+([A-Za-z_]\w*)/gm;
const DEF_RE = /^\s*def\s+([A-Za-z_]\w*)/gm;
const ALL_RE = /__all__\s*=\s*\[([^\]]+)\]/;

function parseFromImports(content: string): Array<{ module: string; names: string[] }> {
...
export function lintPythonExportContractOnDisk(params: {
  ...
  for (const rel of testFiles) {
    ...
    for (const imp of parseFromImports(content)) {
...
export function lintPythonExportContractFromPaths(
  ...
  for (const { testPath, implPath } of pairs) {
    ...
    for (const imp of parseFromImports(testContent)) {
      if (imp.module !== mod) {
        continue;
      }
      for (const name of imp.names) {
