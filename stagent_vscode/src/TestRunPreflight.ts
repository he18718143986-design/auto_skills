...
export type TestRunPreflightIssue = NonNullable<ReturnType<typeof buildMissingTestInfraIssue>>;

export { diskSignalsExpoStack, discoverTestInfraOnDisk, scanTestInfraOnDisk };

/** 是否应对该阶段做 test_run preflight。 */
export function stageNeedsTestRunPreflight(stage: Stage): boolean {
  ...
  if (!cmd || isPythonOnlyTestRunCommand(cmd)) {
    return false;
  }
