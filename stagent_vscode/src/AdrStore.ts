...
/** M24 三门：难逆 / 无上下文会困惑 / 真权衡 —— 至少命中两门才建议落 ADR（全局架构决策除外）。 */
export function shouldCreateAdr(input: ShouldCreateAdrInput): ShouldCreateAdrResult {
  ...
  if (gates.hardToReverse) {
    reasons.push('hard-to-reverse');
  }
  if (gates.confusingWithoutContext) {
    reasons.push('confusing-without-context');
  }
  if (gates.realTradeoff) {
    reasons.push('real-tradeoff');
  }
  const passed = reasons.length;
  const create = isGlobalArchitectureDecisionStage(input.stageId) || passed >= 2;
...
/** 从决策清单（### 章节）构造 ADR 记录；标题优先 stage.title。 */
...
/** 格式化为生成器 prompt 块（warning-only：提示勿重复 litigate 已 accept 条目）。 */
