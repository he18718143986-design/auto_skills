...
test('retryCount increases penalty', () => {
  ...
  assert.ok(computeConfidence(retried).reasons.some((r) => /retried 3/i.test(r)));
});
...
test('output key mismatch lowers confidence', () => {
  ...
  const result = computeConfidence(signals);
  assert.ok(result.reasons.some((r) => /output key/i.test(r)));
});

test('buildConfidenceSignals accepts priorFailurePattern option', () => {
  const signals = buildConfidenceSignals(
    implStage(),
    runtime(),
    'implCode',
    '```python\nx=1\n```',
    goodQuality(),
    { priorFailurePattern: 'tool-execution-failed@stage_impl (2x)' },
  );
  assert.equal(signals.priorFailurePattern, 'tool-execution-failed@stage_impl (2x)');
});
...
test('priorFailurePattern stub applies penalty', () => {
  ...
  assert.ok(withPattern.reasons.some((r) => /prior failure pattern/i.test(r)));
});
...
test('non-code artifact (requirements.txt) not penalized → high confidence, no false pause', () => {
  ...
  assert.ok(!result.reasons.some((r) => /code block/i.test(r)));
});
...
test('missing qualityScore uses neutral prior', () => {
  ...
  const result = computeConfidence(signals);
  assert.ok(result.reasons.some((r) => /neutral prior/i.test(r)));
  assert.ok(result.score >= 0.55);
});
