...
test('createIdleTimeout reset re-arms and only the latest timer survives', () => {
  ...
  idle.reset();
  idle.reset();
  // 多次 reset 后只应剩一个挂起计时器（旧的已清除），避免重复 abort。
  assert.equal(clock.pendingCount(), 1);
  clock.fireAll();
  assert.equal(fired, 1);
});
...
test('selectChatModelsWithTimeout returns models when IPC resolves quickly', async () => {
  const model = { id: 'stub-model' };
  const lm = vscode.lm as unknown as { selectChatModels: (f: unknown) => Promise<unknown[]> };
  const prev = lm.selectChatModels;
  lm.selectChatModels = async () => [model];
  try {
    const out = await selectChatModelsWithTimeout({}, 5000);
    assert.deepEqual(out, [model]);
  } finally {
    lm.selectChatModels = prev;
  }
});

test('selectChatModelsWithTimeout throws lm-select-timeout when IPC hangs', async () => {
  const lm = vscode.lm as unknown as { selectChatModels: (f: unknown) => Promise<unknown[]> };
  const prev = lm.selectChatModels;
  lm.selectChatModels = () => new Promise(() => undefined);
  try {
    await assert.rejects(
      () => selectChatModelsWithTimeout({}, 40),
      (err: unknown) => err instanceof Error && err.message === 'lm-select-timeout',
    );
  } finally {
    lm.selectChatModels = prev;
  }
