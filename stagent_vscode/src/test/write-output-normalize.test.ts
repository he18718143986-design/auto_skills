...
test('normalizeLlmOutputForWritePath prefers dockerfile fence for Dockerfile path', () => {
  const raw = [
    '## Docker setup',
    '',
    '```yaml',
  ...
  const r = normalizeLlmOutputForWritePath('server/Dockerfile', raw);
  assert.equal(r.ok, true);
  if (r.ok) {
    assert.ok(r.content.startsWith('FROM node:20-alpine'));
    assert.ok(!r.content.includes('version:'));
  }
});
