...
import { formatRequiredAnswersValidationError, validateRequiredAnswers } from '../QuestionAfterFlow';
...
test('formatRequiredAnswersValidationError lists question labels', () => {
  const questions: Question[] = [
    { id: 'q1', text: '部署环境', required: true },
    { id: 'q2', text: '数据库版本', required: true },
  ];
  const msg = formatRequiredAnswersValidationError(questions, ['q1', 'q2']);
  assert.match(msg, /部署环境/);
  assert.match(msg, /数据库版本/);
});
...
test('I-7 T2: 用户提交空字符串时回落到 primaryOutputValue（不会出现 outputs.decisionRecord 为空字符串）', () => {
  ...
  assert.equal(rt.outputs.decisionRecord, 'AI 原始决策清单正文');
});
