...
import {
  ...
  detectDangerousShellCommandIssues,
...
test('M38.2: stage_test_run bundled install+jest triggers bundled-install-and-test-run', () => {
  const wf: WorkflowDefinition = {
    id: 'wf',
    version: '2.0',
  ...
  const issues = collectAllCodeRunnerLintIssues(
    'npm install --silent && npx jest',
    wf,
    0,
  );
  assert.ok(issues.some((i) => i.code === 'bundled-install-and-test-run'));
});

test('detectDangerousShellCommandIssues flags rm -rf root', () => {
  const issues = detectDangerousShellCommandIssues('rm -rf /');
  assert.ok(issues.some((i) => i.code === 'dangerous-rm-rf-root'));
});
