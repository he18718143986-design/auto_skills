...
test('input-chat-layout does not override hidden view display', () => {
  ...
  assert.match(html, /\.view\.active\s*\{[^}]*display:\s*flex/);
  assert.match(html, /\.view-bottom-dock/);
});

test('input actions stay docked; generate button disabled until requirement and workspace path are both filled', () => {
  ...
  rt.document.getElementById('user-input')!.oninput?.();
  assert.equal(actions.style.display, 'flex');
  assert.equal(btnGen.disabled, true);

  rt.send({ type: 'taskWorkspacePathPicked', path: '/tmp/ws' });
  assert.equal(actions.style.display, 'flex');
  assert.equal(btnGen.disabled, false);
});
...
test('polish commits user text to history bubble and hides composer dock', () => {
  ...
  assert.ok(dock.className.includes('polish-mode'));
  ...
  assert.equal(rt.document.getElementById('user-message-bubble')!.textContent, '规范用户任务（人工微调）');
