...
export function semanticOfStage(id: string): string | undefined {
  return semanticNameFromTddStageId(id);
}
...
export function findPairedTestStage(
  ...
  return (
    stages.find((s) => s.id === testRunStageIdFromSemanticName(sem)) ??
    stages.find((s) => s.id === testWriteStageIdFromSemanticName(sem))
  );
}
...
export function evaluateRedGreen(input: {
  ...
  if (input.mode === 'off' || !input.pairedTestExists || !input.ranTest) {
    return { outcome: 'pass', reason: gateMsg('redGreen.inactive') };
  }
  if (input.red) {
    return { outcome: 'pass', reason: gateMsg('redGreen.pairedTestRedExpected') };
  }
  const reason = gateMsg('redGreen.pairedTestGreenBeforeImpl');
...
export function isHorizontalTddPlan(stages: Stage[]): boolean {
  ...
  stages.forEach((s, i) => {
    if (isTestWriteStageId(s.id) || isTestRunStageId(s.id)) {
      testIdx.push(i);
    } else if (isImplStageId(s.id)) {
      implIdx.push(i);
    }
  });
