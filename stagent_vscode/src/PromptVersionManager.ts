...
import { getPromptFragmentSlotSeeds } from './generated/PromptFragments';
import { CONTENT_HASH_HEX_PREFIX_CHARS } from './LogPreviewLimits';
import { promptVersionsPath } from './paths/StagentPaths';
import { PROMPT_SLOT_HISTORY_MAX } from './UiListLimits';
...
function hashContent(content: string): string {
  return crypto.createHash('sha256').update(content, 'utf-8').digest('hex').slice(0, CONTENT_HASH_HEX_PREFIX_CHARS);
}
...
function buildInitialStore(): PromptVersionStore {
  const seeds = getPromptFragmentSlotSeeds();
...
export class PromptVersionManager {
  ...
  private persist(next: PromptVersionStore = this.store): void {
    atomicWriteTextFileSync(this.storePath, JSON.stringify(next, null, 2));
    this.store = next;
  }
  ...
  setPrompt(slot: string, content: string, tags: string[] = [], options?: { allowProtected?: boolean }): void {
    ...
    const nextSlot: PromptSlot = existing
      ? {
          ...existing,
          currentVersion: version,
          history: [...existing.history, version].slice(-PROMPT_SLOT_HISTORY_MAX),
        }
      : {
          name: slot,
...
export function resolveDefaultPromptVersionStorePath(workspaceRoot: string): string {
  return promptVersionsPath(workspaceRoot);
}
