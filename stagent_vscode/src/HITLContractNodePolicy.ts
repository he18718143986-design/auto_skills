...
export { DEFAULT_CONTRACT_NODE_PAUSE_THRESHOLD } from './StagentSettingsDefaults';

/**
 * 数据管道核心语义（承载跨模块契约的 impl 节点）。
 *
 * 仅保留**数据变换/搬运专有**词。曾包含 `service`/`controller`/`handler` 等通用应用层词，
 * 会把 `service_worker` / `event_handler` / `*_controller` 等非数据节点误判为契约节点而触发多余
 * HITL 暂停（#9）。这些通用 hub 若真被多方消费，仍由 `isContractNode` 的「≥2 下游引用」主路径捕获，
 * 无需靠名字命中。`main`/`monitor` 保留：prototype 的 main.py/monitor.py 通常是终端装配入口
 * （下游引用为 0，无法靠 ≥2-downstream 捕获），是跨模块契约错位最易暴露处。
 */
export const DATA_PIPELINE_CORE_HINT =
  /(reader|fetcher|parser|loader|analyzer|analyser|aggregator|transformer|writer|exporter|comparator|differ|pipeline|repository|mapper|merger|matcher|calculator|main|monitor)/i;
...
export function isDataContractSourceStage(stage: Stage): boolean {
  ...
  const semantic = semanticNameFromImplStageId(stage.id)?.replace(/^prototype_/, '') ?? '';
