...
import { readLlmTimeoutMs } from '../StagentSettings';
import {
  appendStreamChunk,
...
export function createCoreLlmInvoker(deps: CoreLlmInvokerDeps): CoreLlmInvokeFn {
  async function selectPreferredModels(): Promise<LlmModel[]> {
    return deps.platform.llm.listModels({ family: deps.getPreferredModelFamily() });
  }

  async function selectStructuredModels(): Promise<LlmModel[]> {
    const preferred = await selectPreferredModels();
  ...
  return async function invokeLlmRaw(
    ...
    try {
      ...
      const models = opts?.requireStructured
        ? await selectStructuredModels()
        : await selectPreferredModels();
      if (models.length === 0) {
