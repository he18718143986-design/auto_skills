...
import { CODE_RUNNER_PYTHON_MODULES_PREVIEW_MAX } from './UiListLimits';
import {
...
export function detectPythonImportLintIssues(
  ...
  if (pyCode) {
    ...
    for (const mod of extractPythonTopLevelImports(pyCode)) {
      ...
      const artifactsHint = ` 已登记 artifact：${registry.pythonModules.slice(0, CODE_RUNNER_PYTHON_MODULES_PREVIEW_MAX).join(', ') || '(无 .py)'}`;
      if (verdict === 'flag-file-collision') {
        pushCodedLintIssue(
          issues,
          'python-c-import-not-in-artifacts',
          lintMsg('stagent.lint.pythonImportFileCollision', mod, stageHint, artifactsHint),
        );
      } else {
        pushCodedLintIssue(
          issues,
          'python-c-import-not-in-artifacts',
          lintMsg('stagent.lint.pythonImportMissing', mod, stageHint, artifactsHint),
        );
      }
    }
  }
  ...
  for (const script of extractPythonScriptPaths(command)) {
    ...
    pushCodedLintIssue(
      issues,
      'python-script-not-in-artifacts',
      lintMsg('stagent.lint.pythonScriptMissing', script, stageHint),
    );
  }
