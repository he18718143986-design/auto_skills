...
import type { CodeRunnerConfig, LlmTextConfig, ToolPathBase } from '../workflow-types/StageTypes';
import { isCodeRunnerTool, isLlmTextTool, STAGE_TOOL_CODE_RUNNER } from '../workflow/StageToolKinds';
...
function deriveStartFromEntry(stages: Stage[]): DerivedStart | null {
  ...
  if (pyEntry) {
    return { command: `python3 ${pyEntry}`, pathBase: 'workspace' };
  }
  return null;
}
