...
export function buildExecutionRecoveryMessages(
  ...
  const messages: BackendMessage[] = [
    {
      ...
      ...(instance.status === 'failed' && failedRt?.lastError
        ...
        : instance.status === 'failed'
          ? {
              failedSummary: {
                error: uiMsg('stagent.error.workflowFailedGeneric'),
  ...
  messages.push(...buildStageReplayMessages(instance, instanceKey, stageStatuses));
  return messages;
}
...
export function resetInterruptedExecutionStages(instance: WorkflowInstance): number[] {
  ...
  if (resetIndices.length > 0 && !resolveEffectiveEnableDagScheduler(instance.definition.globalConfig)) {
