...
export function buildApiAlignPromptSuffix(
  ...
  if (isTestWriteStageId(stage.id)) {
    const implFile = pairedImpl ? writeOutputToFileOf(pairedImpl) : undefined;
    if (implFile) {
      const mod = path.posix.basename(implFile.replace(/\\/g, '/'), '.py');
      lines.push(`测试 import 目标：from ${mod} import <上述符号>`);
    }
    lines.push('禁止 invent 未列出的符号名；若与 impl 已落盘代码冲突，以本列表与决策清单为准。');
  } else {
    lines.push('实现代码中的 def/class 名称必须与上表一致，供后续 test_write 直接 import。');
  }
