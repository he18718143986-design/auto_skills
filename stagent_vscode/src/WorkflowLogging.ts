...
import { SESSION_DEBUG_MAX_BYTES } from './SessionDebugLog';
...
export function sanitizeForLog(value: unknown): unknown {
  ...
  if (typeof value === 'string') {
    return value.length > LOG_PREVIEW_MEDIUM ? `${value.slice(0, LOG_PREVIEW_MEDIUM)}...` : value;
  }
...
export function appendDebugLogLine(taskDir: string, line: string): void {
  const debugPath = taskDebugLogPath(taskDir);
  fs.mkdirSync(taskDir, { recursive: true });
  appendLogLineAtomicSync(debugPath, line, { maxBytes: SESSION_DEBUG_MAX_BYTES });
}
