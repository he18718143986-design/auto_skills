/**
 * LLM 调用辅助（超时、提示拼装、模型枚举）。
 *
 * **空闲 / 总时长契约**（`LlmClient.buildLlmRequestModelOptions` 注入）：
 * - `modelOptions.onActivity`：任意响应活动（含 reasoning）时调用，用于重置空闲计时；
 * - `modelOptions.maxRequestMs`：墙钟上限（毫秒），与 `readLlmTimeoutMs()` / `stagent.llmTimeoutSeconds` 一致。
 * 上述字段同时适用于 `vscode.lm` 流式路径与 {@link OpenAiCompatibleLlm} 的 fetch 路径。
 */
import * as vscode from 'vscode';
import { sleepMs } from './FsAsync';

/** 与 `package.json` → `stagent.llmTimeoutSeconds` 默认值一致 */
export const DEFAULT_LLM_TIMEOUT_SECONDS = 180;

/** `vscode.lm.selectChatModels` IPC 默认上限（与流式空闲超时不同）。 */
export const DEFAULT_LM_SELECT_TIMEOUT_MS = 15_000;

/** 侧栏 / onboarding 等 UI 触发的模型枚举上限。 */
export const UI_LM_SELECT_TIMEOUT_MS = 8_000;
...
/** `vscode.lm.selectChatModels` 带上限，避免 Copilot IPC 卡死拖住 Extension Host。 */
export async function selectChatModelsWithTimeout(
  filter: vscode.LanguageModelChatSelector,
  timeoutMs: number = DEFAULT_LM_SELECT_TIMEOUT_MS,
): Promise<vscode.LanguageModelChat[]> {
  return Promise.race([
    vscode.lm.selectChatModels(filter),
    sleepMs(timeoutMs).then(() => {
      throw new Error('lm-select-timeout');
    }),
  ]);
}
