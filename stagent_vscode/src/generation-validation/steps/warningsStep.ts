...
import { collectGenerateWarningMessages } from '../../QualityGateRunner';
...
export async function runWarningsStep(
  ...
  const warnings = [
    ...(await collectGenerateWarningMessages({
      phase: 'generate',
      workflow: wf,
      effectiveTaskType: ctx.effectiveType,
      uiTaskType: ctx.uiTaskType,
      modelTaskType: ctx.modelTaskType,
      userInput: ctx.userInput,
      taskWorkspaceAbs: ctx.taskWorkspaceAbs,
      depGraph: ctx.depGraph,
      complexity: ctx.complexity,
      generationGates: ctx.gates,
      verifyResult,
