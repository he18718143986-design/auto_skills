...
export function ensureInstanceBound(
  ...
  if (!key) {
    void vscode.window.showWarningMessage(uiMsg('stagent.warn.instanceNotBound'));
    return false;
  }
  const loaded = host.loadInstanceByKey(key);
  if (!loaded) {
    void vscode.window.showWarningMessage(uiMsg('stagent.warn.instanceNotFound'));
    return false;
  }
  const activated = tryActivateInstance(host, key, loaded, panel, { pushRecoveryUi: true });
  if (!activated.ok) {
    void vscode.window.showWarningMessage(uiMsg('stagent.warn.reason', activated.reason));
    return false;
  }
