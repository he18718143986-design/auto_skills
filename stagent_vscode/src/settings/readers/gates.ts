...
export function readRedGreenGateMode(cfg?: vscode.WorkspaceConfiguration): RedGreenMode {
...
export function readDebugFeedbackLoopMode(cfg?: vscode.WorkspaceConfiguration): DebugFeedbackLoopMode {
...
export function readDebugFeedbackLoopRuntimeHard(cfg?: vscode.WorkspaceConfiguration): boolean {
  return readDebugFeedbackLoopMode(cfg) === 'hard';
}

/** @deprecated 使用 readDebugFeedbackLoopRuntimeHard */
export function readDebugRequireFeedbackLoop(cfg?: vscode.WorkspaceConfiguration): boolean {
...
export function readPlanCompletenessGateEnabled(cfg?: vscode.WorkspaceConfiguration): boolean {
...
export function readPlanStructuralRepairMode(cfg?: vscode.WorkspaceConfiguration): PlanStructuralRepairMode {
...
export function readRuntimeRule20VerifyEnabled(cfg?: vscode.WorkspaceConfiguration): boolean {
  return readConfigBooleanDefaultTrue(cfg, 'enableRuntimeRule20Verify');
}

/** Phase 4（可选）：to-issues horizontal-tdd 观测升 fail；默认 false */
export function readToIssuesHorizontalLayeringFail(cfg?: vscode.WorkspaceConfiguration): boolean {
  return readConfigBooleanStrictTrue(cfg, 'toIssues.horizontalLayeringFail');
}

/** M18.2：静态分析管道接入热路径；默认 false（灰度） */
export function readStaticAnalysisEnabled(cfg?: vscode.WorkspaceConfiguration): boolean {
...
export function readGrillAdaptiveModeEnabled(cfg?: vscode.WorkspaceConfiguration): boolean {
...
export function readGlossaryEnabled(cfg?: vscode.WorkspaceConfiguration): boolean {
  return readConfigBooleanDefaultTrue(cfg, 'glossary.enabled');
}

/** vscode `stagent.architecture.depthScoring`；M25，默认 false（深模块评分接入质量分） */
export function readArchitectureDepthScoringEnabled(cfg?: vscode.WorkspaceConfiguration): boolean {
