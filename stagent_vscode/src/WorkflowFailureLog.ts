...
import { appendJsonlLineAtomicSync } from './jsonl/JsonlAtomicAppend';
import { globalFailureLogsDir, GLOBAL_FAILURES_FILENAME, WF_FAILURES_FILENAME } from './paths/StagentPaths';
...
export function appendWorkflowFailureJsonl(
  ...
  try {
    const filePath = path.join(taskDir, WF_FAILURES_FILENAME);
    fs.mkdirSync(taskDir, { recursive: true });
    appendJsonlLineAtomicSync(filePath, JSON.stringify(record));
  } catch (e) {
...
export function appendGlobalFailureJsonl(
  ...
  try {
    const dir = globalFailureLogsDir(globalStoragePath);
    fs.mkdirSync(dir, { recursive: true });
    appendJsonlLineAtomicSync(path.join(dir, GLOBAL_FAILURES_FILENAME), JSON.stringify(record));
  } catch (e) {
