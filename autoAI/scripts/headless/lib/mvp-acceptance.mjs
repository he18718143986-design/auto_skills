...
/** 需求关键词 → 工作区符号/路径 traceability（启发式）。 */
export const TRACEABILITY_RULES = [
  {
    id: 'index-resonance',
    keywords: ['指数共振', '上证指数', '深证成指'],
    check: (ws, readText) => {
      const hay = readText(['signals', 'tests'])
      return /上证|深证|指数|index/i.test(hay)
    },
  ...
  },
  {
    ...
    keywords: ['15 点', '对冲', '止损'],
    check: (ws, readText) => {
      const riskDir = path.join(ws, 'risk')
      if (!dirHasPy(riskDir)) return false
      const hay = readText(['risk', 'tests'])
      return /hedge|stop_loss|止损|对冲/i.test(hay)
    },
    hint: 'risk/ 非空且 tests/ 或 risk/ 含 hedge/stop_loss 相关符号',
  },
  {
    id: 'sim-broker',
    keywords: ['SimBroker', 'BrokerAdapter'],
    check: (ws, readText) => {
      const hay = readText(['broker', 'src'])
      return /SimBroker|BrokerAdapter/.test(hay)
    },
 ...
 * @param {{ outcome?: string, requireTraceability?: boolean }} opts
 */
