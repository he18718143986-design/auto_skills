...
test('S4 hybrid：prependGrillStage 在引擎 impl 工作流前插入 native grill，整体仍 engine-valid 且非 skill-native', async () => {
  const { prependGrillStage } = await import('../SkillWorkflowAssembler');
  const { validateGeneratedWorkflow } = await import('../WorkflowValidation');
  const { isSkillNativeWorkflow } = await import('../SkillToolKinds');

  // 一个「引擎 LLM 生成」式的 software 工作流（决策+impl+test 链），先确保自身合法
  const implWf: WorkflowDefinition = {
    id: 'impl',
    version: '2.0',
    ...
    stages: [
      ...
      },
      {
        ...
        title: '实现 auth',
        tool: 'llm-text',
        toolConfig: { type: 'llm-text', systemPrompt: 'x', writeOutputToFile: 'auth.ts' },
        input: { sources: [], mergeStrategy: 'concat' },
  ...
  assert.deepEqual(validateGeneratedWorkflow(implWf), []);
  ...
  const hybrid = prependGrillStage(implWf, grillSkill, { userTask: 'u' });

  // grill 在最前，且为决策阶段（native 判断 + HITL）
  assert.equal(hybrid.stages[0].id, 'stage_skill_grill_with_docs');
  assert.equal(hybrid.stages[0].isDecisionStage, true);
  // impl 段保留 → 仍能产出软件文件
  assert.ok(hybrid.stages.some((s) => s.id === 'stage_impl_auth'));
  // 不是 skill-native（impl 段仍受引擎约束）
  assert.equal(isSkillNativeWorkflow(hybrid), false);
  // 整体仍通过引擎结构 + 不变式校验
  assert.deepEqual(validateGeneratedWorkflow(hybrid), []);
  // 幂等：重复 prepend 不重复插入
  assert.equal(prependGrillStage(hybrid, grillSkill).stages.length, hybrid.stages.length);
});
