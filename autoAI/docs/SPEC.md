...
| **TA-03** | 已在登录页完成账号登录 → 聊天输入框在后台变为可见 | 500ms 首检或 3s 轮询命中 → 登录页自动收起，跳转 ChatPage，Tab Bar 出现该站点标签，status=connected | [手工]* |
...
> *TA-03 `[手工]*`：需真实账号完成登录或高精度 mock 登录页（实现成本高于收益）；其他 `[E2E]` 条目均可用 `e2e/fixtures/mock-site` 配合 Playwright 实现。
...
> **mock-site 说明**：`e2e/fixtures/mock-site` 已提供可本地运行的简单 HTML 站点，可扩展支持 quotaExhaustedIndicator、多 siteId 场景，无需真实 AI 账号即可执行大多数回归用例。
 ...
 * 登录成功检测策略：每 3 秒对页面执行 inputSelectors[0].selector 的 querySelector，
 * 若返回非空可见元素则判定登录完成，最长等待 5 分钟。
...
| `chat:send` | 渲染→主 | 发送消息（提供 `siteId` + `text` + 可选 `attachments` 路径数组；M12 后需增加 `conversationId: string`） |
...
| `dialog:open-file` | 渲染→主 | M11：触发系统文件选择对话框，返回 `{ paths: string[] }`；用于附件上传前获取本地文件路径 |
...
**关闭方式**：  
- 自动：主进程每 3 秒轮询 `inputSelector`，元素可见即判定登录成功 → 自动收起，站点切换控件新增该账号，status → `connected`  
- 最长等待 5 分钟超时后停止轮询
...
### M2：能打开网站（连接）
- [ ] `session.ts` — 持久化 Session
- [ ] `browser-view.ts` — 在应用内显示 WebContentsView
- [ ] `site:add` / `site:open-login` IPC
- [ ] ResourcesPage 显示网站列表 + 登录按钮
...
### M4：能收回复（检测）
- [ ] `response-watcher.ts` — MutationObserver + 稳定检测
- [ ] `chat:reply` IPC 推送
- [ ] MessageList 显示 AI 回复

### M5：处理失败（校准 + Selector Debugger）
- [ ] `CalibrationOverlay.tsx` — 引导用户点击输入框和回复区域（两步流程）
- [ ] `site-store.ts` 保存校准结果
- [ ] `calibrate:start` / `calibrate:done` IPC
- [ ] `SelectorDebugger.tsx` — 5 个字段直接编辑，支持保存和「重新校准」入口
- [ ] `site:update-selectors` IPC
- [ ] 额度耗尽检测（`quotaExhaustedIndicator` 并行轮询）
- [ ] `chat:quota-exhausted` IPC + SiteCard 🟠 额度用尽状态
